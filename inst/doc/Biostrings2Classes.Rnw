...
The following is the same as doing \Robject{subject(v5)}
...
v5[c(TRUE, FALSE)]
@
Note that the logical vector is recycled to the length of \Robject{v5}.
...
Note that \Robject{first(v5)[i]} is equivalent to
\Robject{first(v5[i])}, except that the former will not issue
...
\Robject{width(v5)[i]} is equivalent to \Robject{length(v5[[i]])}
except that the former will not issue an error
if \Robject{i} is out of bounds or if view \Robject{v5[i]}
is {\it out of limits}.
