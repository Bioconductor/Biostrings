...
objects is not efficient and one should always use the \Rmethod{subBString}
method to extract a substring from a big string:
<<a4>>=
bb <- subBString(b, 3, 6)
...
BString objects. It will be essentially doing toString(subBString()).]}
...
When both sides are \Rclass{BString} (or \Rclass{DNAString}) objects
then the comparison is very fast because it only has to call the C standard
...
<<b3>>=
dAlphabet <- DNAString("ACGT-MRSVWYHKDBN")
rAlphabet <- RNAString("UGCA-KYSBWRDMHVN")
dAlphabet == rAlphabet
@
Note that this behaviour is consistent with the fact that
\Robject{RNAString(dAlphabet)} is equal to \Robject{rAlphabet} and
\Robject{DNAString(rAlphabet)} is equal to \Robject{dAlphabet}.
...
This is the same as doing \Robject{views(d, 1, length(d))}:
<<d2,results=hide>>=
views(d)
@
...
\section{The \Rmethod{first}, \Rmethod{last} and \Rmethod{width}
methods}
...
(same for \Rmethod{last} and \Rmethod{width} methods).
