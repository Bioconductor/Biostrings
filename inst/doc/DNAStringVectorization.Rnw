...
\section{\Rfunction{DNAString} vs \Rfunction{DNAStringViews}}
...
\section{The \Rfunction{DNAStringViews} function}
...
\Rfunction{DNAStringViews} function:
<<b1>>=
dnav <- DNAStringViews(c("TTGAAAA-C", "TC-N"))
dnav
@
...
<<c2>>=
system.time(z <- DNAStringViews(hgu95av2probe$sequence))
z
@
...
%src <- sapply(1:length,
%              function(i) {
%                paste(sample(alphabet, 250, replace=TRUE), collapse="")
%              })
%system.time(dna2v <- DNAStringViews(src))
%dna2v
%@
%
%With \Rpackage{Biostrings}~1, the call to
...
\section{Loading a FASTA file in a \Rclass{DNAStringViews} object}

The \Rfunction{DNAStringViews} function can be used to load a FASTA file
in a \Rclass{DNAStringViews} object:
<<d1>>=
f <- system.file("Exfiles", "someORF.fsa", package="Biostrings")
orf <- DNAStringViews(file(f))
orf
desc(orf)
@
...
The RNA counterpart of the \Rfunction{DNAStringViews} function is
the \Rfunction{RNAStringViews} functions.
Both can be used to switch between ``DNA'' and ``RNA'' views
on the same string:
<<e1>>=
orf2 <- RNAStringViews(orf)
@
