...
Here is how we can write the functions that will perform this analysis:
<<c1>>=
...
writeHits <- function(seqname, matches, strand, patternID, file="")
{
    for (i in seq_len(length(matches))) {
        cat(seqname, start(matches)[i], end(matches)[i], strand, patternID,
            file=file, sep="\t", append=TRUE)
        cat("\n", file=file, append=TRUE)
    }
}

runAnalysis1 <- function(outfile="")
{
    library(BSgenome.Celegans.UCSC.ce2)
    ce2dict0_file <- system.file("extdata/ce2dict0.fsa", package="Biostrings")
    cat("Input dictionary: ", ce2dict0_file, "\n", sep="")
    ce2dict0 <- read.BStringViews(ce2dict0_file, "fasta", subjectClass="DNAString")
    seqnames <- seqnames(Celegans)
    seq_list <- paste(seqnames, collapse=", ")
    cat("Target: ", providerVersion(Celegans), " chromosomes ", seq_list, "\n", sep="")
    if (outfile != "")
        writeHeader(file=outfile)
    for (seqname in seqnames) {
        subject <- Celegans[[seqname]]
        cat(">>> Finding all hits in chromosome ", seqname, " ...\n", sep="")
        for (i in seq_len(length(ce2dict0))) {
            patternID <- desc(ce2dict0)[i]
            pattern <- ce2dict0[[i]]
            plus_matches <- matchPattern(pattern, subject)
            writeHits(seqname, plus_matches, "+", patternID, file=outfile)
            pattern <- reverseComplement(pattern)
            minus_matches <- matchPattern(pattern, subject)
            writeHits(seqname, minus_matches, "-", patternID, file=outfile)
        }
        cat(">>> DONE\n")
...
Run the analysis and put the results in \Robject{"ce2dict0_ana1.txt"} with:
<<c2>>=
runAnalysis1("ce2dict0_ana1.txt")
@

Here is some very simple example of post analysis:
<<c3>>=
hits1 <- read.table("ce2dict0_ana1.txt", header=TRUE)
nrow(hits1) # total number of hits
table(hits1$seqname) # hits per chromosome
hits1_table <- table(hits1$patternID)
hits1_table # hits per pattern
...
Here is a function that can be used to plot the hits:
<<c4>>=
