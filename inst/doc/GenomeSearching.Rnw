...
<<c1>>=
writeMatches <- function(patternID, seqname, matches, strand, outfile)
{
    for (i in seq_len(length(matches))) {
        cat(patternID, seqname, start(matches)[i], end(matches)[i], strand,
            file=outfile, sep="\t", append=TRUE)
        cat("\n", file=outfile, append=TRUE)
    }
}

runAnalysis <- function(outfile)
{
    ...
    for (seqname in seqnames) {
        for (i in seq_len(length(ce2dict0))) {
            pattern <- ce2dict0[[i]]
            plus_matches <- matchPattern(pattern, Celegans[[seqname]])
            writeMatches(desc(ce2dict0)[i], seqname, plus_matches, "+", outfile)
            pattern <- reverseComplement(pattern)
            minus_matches <- matchPattern(pattern, Celegans[[seqname]])
            writeMatches(desc(ce2dict0)[i], seqname, minus_matches, "-", outfile)
        }
