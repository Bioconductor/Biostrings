...
Display the number of patterns in the dictionary and then the first five
patterns and their IDs with:
<<c2>>=
length(ce2dict0)
ce2dict0[1:5]
desc(ce2dict0)[1:5]
...
writeHits <- function(seqname, starts, ends, strand, patternID, file="")
{
    for (i in seq_len(length(starts))) {
        cat(seqname, starts[i], ends[i], strand, patternID,
...
runAnalysis1 <- function(dict0, outfile="")
{
    ...
    for (seqname in seqnames) {
        ...
        for (i in seq_len(length(dict0))) {
            ...
            writeHits(seqname, start(plus_matches), end(plus_matches),
                      "+", patternID, file=outfile)
            pattern2 <- reverseComplement(pattern)
            minus_matches <- matchPattern(pattern2, subject)
            writeHits(seqname, start(minus_matches), end(minus_matches),
                      "-", patternID, file=outfile)
        }
        cat(">>> DONE\n")
...
\begin{itemize}
  ...
  \Robject{pattern2 <- reverseComplement(pattern)}) and NOT by
...
runAnalysis2 <- function(dict0, outfile="")
{
    ...
    for (seqname in seqnames) {
        ...
        plus_ends <- as.list(matchPDict(pdict, subject))
        for (i in seq_len(length(plus_ends))) {
            ends <- plus_ends[[i]]
            if (length(ends) == 0)
                next
            starts <- ends - width(pdict) + 1
            patternID <- desc(dict0)[i]
            writeHits(seqname, starts, ends, "+", patternID, file=outfile)
        }
        cat(">>> DONE\n")
        unload(Celegans, seqname)
    }
