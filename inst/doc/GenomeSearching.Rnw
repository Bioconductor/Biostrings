...
%   > write.BStringViews(BStringViews(ce2dict0, "DNAString"), file="ce2dict0.fa", format="fasta")
...
<<c1>>=
ce2dict0_file <- system.file("extdata/ce2dict0.fa", package="Biostrings")
...
writeHits <- function(seqname, matches, strand, file="")
{
    hits <- data.frame(seqname=rep.int(seqname, length(matches)),
                       ...
                       patternID=desc(matches),
                       check.names=FALSE)
    write.table(hits, file=file, append=TRUE, quote=FALSE, sep="\t",
...
runAnalysis1 <- function(dict0, outfile="")
{
    ...
    for (seqname in seqnames) {
        ...
        for (i in seq_len(length(dict0))) {
            ...
            desc(plus_matches) <- rep.int(patternID, length(plus_matches))
            writeHits(seqname, plus_matches, "+", file=outfile)
            rcpattern <- reverseComplement(pattern)
            minus_matches <- matchPattern(rcpattern, subject)
            desc(minus_matches) <- rep.int(patternID, length(minus_matches))
...
runAnalysis2 <- function(dict0, outfile="")
{
    ...
    oneStrandAnalysis <- function(dict0, strand)
    {
        pdict <- new("ULdna_PDict", dict0)
        for (seqname in seqnames) {
            subject <- Celegans[[seqname]]
            cat(">>> Finding all hits in strand", strand, "of chromosome", seqname, "...\n")
            p2v <- matchPDict(pdict, subject)
            matches <- extractAllMatches(subject, p2v)
            writeHits(seqname, matches, strand, file=outfile)
            cat(">>> DONE\n")
            unload(Celegans, seqname)
        }
    }

    oneStrandAnalysis(dict0, "+")
    rcdict0 <- reverseComplement(dict0)
    oneStrandAnalysis(rcdict0, "-")
}
@
