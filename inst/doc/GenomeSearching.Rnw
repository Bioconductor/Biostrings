...
the nucleotide sequence of the {\it reverse} strand (aka {\it indirect}
...
sequence by taking its reverse complement.
...
sequence is almost never needed hence computing it by applying
\Rfunction{reverseComplement} to the {\it forward} strand sequence
should be avoided at all cost!
In fact, a {\it reverse} strand analysis can always be transposed into
a {\it forward} strand analysis and this is indeed the right thing to
do. See further in this document for how to find arbitrary patterns in the
{\it reverse} strand of a chromosome.
...
%   > write.BStringViews(BStringViews(ce2dict0, "DNAString"), file="ce2dict0.fsa", format="fasta")
  ...
  in a genome} section below for the special case where all the patterns have
  the same length.
