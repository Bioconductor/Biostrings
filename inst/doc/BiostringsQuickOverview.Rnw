...
\hline
\Rfunction{stackStrings} & Turn a variable-width set of strings into a
    ``rectangular'' set (i.e. constant-width), by clipping and/or padding
    the strings, after shifting them horizontally. \\
\hline
\end{tabular}
\end{center}
