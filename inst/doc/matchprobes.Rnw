...
%\VignetteDepends{Biostrings, hgu95av2probe, hgu95av2cdf, affy}
...
pdict <- PDict(pm)
subject <- pm[3001:4000]
m <- vcountPDict(pdict, subject)
table(colSums(m))
which(colSums(m) == 3)
ii <- which(m[ , 877] != 0)
pm[ii]
...
The base content (number of occurence of each character) of the
...
{
    parts <- threebands(DNAStringSet(probes), start=13, end=13)
    xscat(parts$left, complement(parts$middle), parts$right)
}
mm <- pm2mm(pm)
...
occurences of each of the four bases A, C, G, T in each probe sequence.
<<bc>>=
...
And let us create an \Rclass{AffyBatch} object. (WARNING:
\Rpackage{matchprobes} is deprecated so CL2001032020AA.cel
will be moved to a new home soon!)
<<abatch>>=
library(matchprobes)
f <- system.file("extdata", "CL2001032020AA.cel", package="matchprobes")
