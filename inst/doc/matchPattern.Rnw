...
If needed, the \Rmethod{countPattern} method converts its first argument
(the pattern) to an object of the same class than its second argument
(the subject) before it passes it to the fast algorithm.
