...
Here we load the {\tt someORF.fa} file into a \Rclass{DNAStringSet} object
by using the \Rfunction{read.DNAStringSet} function:
<<gety>>=
library(Biostrings)
...
a substitution matrix appropriate for the alphabets from which
...
The \Rfunction{consmat} function is provided for computing
a consensus matrix for a set of equal-length strings
