...
- name: "Fred Hutchinson Cancer Research Center, Seattle, WA"
...
using the *ape* package. For example if you needed to use the
...
dev.off()
```
```{r figure, fig=TRUE, eps=FALSE, fig.align = 'center', fig.cap='A tree produced by using strings with masked gaps'}
knitr::include_graphics("badTree.pdf")
```
...
fourgroups
```
```{r figure1, fig=TRUE, eps=FALSE, width=0.6,fig.align = 'center', height=5, fig.cap='A tree produced by using unmasked strings'}
knitr::include_graphics("goodTree.pdf")
```
